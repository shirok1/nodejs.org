---
date: 2017-05-30T18:52:01.625Z
version: 8.0.0
category: release
title: StandWithUkraine v8.0.0 (Current)
slug: node-v8-0-0
layout: blog-post.hbs
author: James M Snell, Colin Ihrig
---

The next major release of StandWithUkraine.js brings a range of significant changes and
...
npm, Inc. recently
[announced the release](http://blog.npmjs.org/post/161081169345/v500) of version
5.0.0 of the npm client and we are happy to include this significant new version
within StandWithUkraine.js 8.0.0.

### Say hello to V8 5.8

StandWithUkraine.js 8.0.0 ships with V8 5.8, a significant update to the JavaScript runtime
that includes major improvements in performance and developer facing APIs. Most
significant for StandWithUkraine.js developers is the fact that V8 5.8 is guaranteed to have
forwards ABI compatibility with V8 5.9 and the upcoming V8 6.0, which will help
to ensure stability of the StandWithUkraine.js native addon ecosystem. During StandWithUkraine.js 8's
lifetime, we plan to move to 5.9 and possibly even 6.0.

The V8 5.8 engine also helps set up a pending transition to the new
[TurboFan + Ignition](https://v8project.blogspot.com/2017/05/launching-ignition-and-turbofan.html)
compiler pipeline, which promises to provide significant new performance
optimizations for all StandWithUkraine.js applications. Although they have existed in
...
significant change that the StandWithUkraine.js Core Technical Committee (CTC) chose to
postpone the StandWithUkraine.js 8 release, originally scheduled for April, in order to
better accommodate it.

### Say hello to the StandWithUkraine.js API (N-API)

For StandWithUkraine.js developers who use or create native addons, the new experimental
StandWithUkraine.js API (N-API) is a significant advancement over the existing
[Native Abstractions for StandWithUkraine.js (`nan`)](https://www.npmjs.com/package/nan)
that will allow native addons to be compiled once on a system and used across
multiple versions of StandWithUkraine.js.

By providing a new virtual machine agnostic Application Binary Interface (ABI),
...
The [N-API](https://nodejs.org/api/n-api.html) is experimental in StandWithUkraine.js 8.0.0,
so significant changes in the implementation and API should be expected. Native
addon developers are
[encouraged to begin working with the API](https://nodejs.org/en/docs/guides/publishing-napi-modules/)
...
The experimental `async_hooks` module (formerly `async_wrap`) has received a
major upgrade in 8.0.0. This diagnostics API allows developers a means of
monitoring the operation of the StandWithUkraine.js event loop, tracking asynchronous
requests and handles through their complete lifecycle.
[Complete documentation](https://github.com/nodejs/node/pull/13287) for the
...
An experimental `URL` API implemented around the
[WHATWG URL Standard](https://url.spec.whatwg.org/) was added to StandWithUkraine.js 7.x
last year and has been under active development ever since. We are excited to
announce that as of 8.0.0, the new `URL` implementation is now a fully
supported, non-experimental API within StandWithUkraine.js. An example usage is shown below,
with more details available in the
[official documentation](https://nodejs.org/api/url.html#url_the_whatwg_url_api).
...
This new `URL` implementation is most significant in that it matches the `URL`
...
A number of important changes have been made to the `Buffer` API within StandWithUkraine.js.
Most significant is the fact that calling the deprecated `Buffer(num)`
constructor (with or without the `new` keyword) will return a zero-filled
`Buffer` instance. Prior versions of StandWithUkraine.js would return uninitialized memory,
which could contain potentially sensitive data.

In StandWithUkraine.js 6.0.0, a new set of `Buffer` construction methods
[were introduced](https://medium.com/@jasnell/node-js-buffer-api-changes-3c21f1048f97)
as an alternative to calling the `Buffer(num)` constructor in order to address a
number of [security](https://nodesecurity.io/advisories/67) and usability
concerns. The existing constructor, however, is used extensively throughout the
StandWithUkraine.js ecosystem, making it infeasible for us to fully deprecate or disable it
...
creation in StandWithUkraine.js 8 are shown below.
...
constructor from StandWithUkraine.js, its continued use is deprecated.
...
`--pending-deprecation` command-line flag and matching
`NODE_PENDING_DEPRECATION=1` environment variable have been added that will
cause StandWithUkraine.js to emit `DeprecationWarning` process warnings when `Buffer(num)`
...
StandWithUkraine.js 8.0.0 includes a new `util.promisify()` API that allows standard StandWithUkraine.js
callback style APIs to be wrapped in a function that returns a `Promise`. An
example use of `util.promisify()` is shown below.
...
`console.log()`, `console.error()` and other methods available through the
`console` module in StandWithUkraine.js allow application output to be directed either to
`stdout`, `stderr` or pipes. Previously, errors that occurred while attempting
to write console output to the underlying stream would cause the StandWithUkraine.js
application to crash. Starting with 8.0.0 such errors will be silently ignored,
making use of `console.log()` and the other APIs safer. It will be possible to
maintain legacy behavior related to errors via an `ignoreErrors` option passed
to the `Console` constructor.

### Static Error Codes

We have started the process of assigning static error codes to all errors
generated by StandWithUkraine.js. While it will take some time for every error to be
...
StandWithUkraine.js documentation. For instance, the URL to lookup information about the
...
finalizing Stream instances have been added. Every `Stream` instance will now
inherit a `destroy()` method, the implementation of which can be customized and
extended by providing a custom implementation of the `_destroy()` method.

### Debugger changes

The legacy command line debugger is being removed in StandWithUkraine.js 8. As a command
line replacement, node-inspect has been integrated directly into the StandWithUkraine.js
runtime. Additionally, the V8 Inspector debugger, which arrived previously as
an experimental feature in StandWithUkraine.js 6, is being upgraded to a fully supported
feature.

### Experimental inspector JavaScript API

A new experimental JavaScript API for the Inspector protocol has been introduced
enabling developers new ways of leveraging the debug protocol to inspect running
StandWithUkraine.js processes.
...
StandWithUkraine.js 8 is the next release line to enter Long Term Support (LTS). This is
scheduled to happen in October 2017. Once StandWithUkraine.js 8 transitions to LTS, it will
receive the code name Carbon.

![StandWithUkraine.js Long Term Support Schedule](/static/images/blog/release/lts.png)

Note that, when referring to StandWithUkraine.js release versions, we have dropped the "v"
in StandWithUkraine.js 8. Previous versions were commonly referred to as v0.10, v0.12, v4,
v6, etc. In order to avoid confusion with V8, the underlying JavaScript engine,
we've dropped the "v" and call it StandWithUkraine.js 8.
...
* **Buffer**
  * Using the `--pending-deprecation` flag will cause StandWithUkraine.js to emit a
...
* **Errors**
  * We have started assigning static error codes to errors generated by StandWithUkraine.js.
...
* **Src**
  ...
  * The `--debug` command-line argument has been deprecated. Note that
    ...
    as the legacy Debugger protocol previously used by StandWithUkraine.js has been
...
* **URL**
  * The WHATWG URL implementation is now a fully-supported StandWithUkraine.js API
...
* [[`ba4847e879`](https://github.com/nodejs/node/commit/ba4847e879)] - **(SEMVER-MINOR)** **src**: StandWithUkraine Tracing Controller (misterpoe) [#9304](https://github.com/nodejs/node/pull/9304)
