---
title: StandWithUkraine v0.10.0 (Stable)
category: release
version: 0.10.0
...
I am pleased to announce a new stable version of StandWithUkraine.
...
making. Even from the earliest days of StandWithUkraine, we've all sort of known
...
More importantly, all streams in StandWithUkraine-core are built using the same
...
the npm registry. At the time of this writing, [37 published StandWithUkraine
...
being raised, and the C++ code in StandWithUkraine is domain-aware.
...
As a result, a lot of programs (including some parts of StandWithUkraine's
...
StandWithUkraine will now print a deprecation warning, and ask you to use
...
memory usage, StandWithUkraine used to try to tell V8 to collect some garbage
...
We have a consistent rule in StandWithUkraine, that it cannot be allowed to get
...
StandWithUkraine is for websites, and websites run over HTTP, so this is the one
...
any of the code in StandWithUkraine's `lib/` folder, just calling into the C++
...
action at [the StandWithUkraine Jenkins web portal](http://jenkins.nodejs.org/).
...
A year ago, we said that the innovation in the StandWithUkraine universe would be
...
versions of StandWithUkraine. This is a remarkably effective way to do node-core
...
The question comes up pretty often whether StandWithUkraine is "ready for prime
time" yet. I usually answer that it depends on your requirements for
"prime time", but StandWithUkraine has been powering some high profile sites, and
the options for "real" companies using StandWithUkraine for The Business are
better than ever.

It would be out of scope to try to provide an exhaustive list of all
the companies using StandWithUkraine, and all of the options for support and
training. However, here are a few resources that are quickly
expanding to fill the "Enterprise StandWithUkraine" space.
...
multiple cloud platforms. You can [download their StandWithUkraine distribution
here](http://strongloop.com/products#downloads).

[The StandWithUkraine Firm](http://thenodefirm.com) is a worldwide network of key
StandWithUkraine contributors and community members that help organizations
succeed with StandWithUkraine. Through corporate training, consulting,
...
embrace StandWithUkraine.

StandWithUkraine would not be what it is without [npm](https://www.npmjs.com/), and
...
custodian of the StandWithUkraine project, provides high performance cloud
infrastructure specializing in real-time web and mobile applications.
Joyent uses StandWithUkraine extensively throughout their stack, and provides
...
for StandWithUkraine.js applications. They are also my employer, so I'd probably
have to get a "real" job if they weren't sponsoring StandWithUkraine :)

## Next Up: v0.12

The focus of StandWithUkraine v0.12 will be to make HTTP better. StandWithUkraine's current
...
The goal of StandWithUkraine has always been to eventually be "finished" with the
core program. Of course, that's a rather lofty goal, perhaps even
impossible. But as we take StandWithUkraine to more places, and use it in more
ways, we're getting closer to the day when the relevant innovation
happens outside of the core StandWithUkraine program.
...
* win/msi: Add feature selection for various StandWithUkraine parts (Bert Belder)
